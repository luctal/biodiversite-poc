...
def compute_weekly_shannon_distribution(df_input):
    ...
    pour le jeu de données filtré.

    Cette fonction sert à mesurer la stabilité temporelle :
    - on regroupe les observations par site, semaine et espèce
    - on calcule ensuite Shannon semaine par semaine
    ...
    # Agrégation par site / semaine / espèce
    df_weekly_stats = df_input.groupby(
        ['site', 'Semaine', 'vernacular_name']
...
def compute_weekly_shannon_distribution(df_input):
    """
    Calcule la distribution hebdomadaire de Shannon
    pour évaluer la stabilité temporelle.
    """
    if df_input.empty:
        return pd.DataFrame(columns=['site', 'Semaine', 'shannon_val'])
